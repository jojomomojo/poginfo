...
$ helm upgrade -i my-release poginfo/poginfo 
```

The command deploys poginfo on the Kubernetes cluster in the default namespace.
...
The following tables lists the configurable parameters of the poginfo chart and their default values.
...
`image.repository` | `jojomomojo/poginfo` | Image repository
...
`service.grpcService` | `poginfo` | gPRC service name
...
```console
$ helm install my-release poginfo/poginfo \
...
```console
$ helm install my-release poginfo/poginfo -f values.yaml
```
